...
| 10 | JSON-RPC API surface | **DONE — 3 fixes** | Fixed `6f3a73c55`: APIv1 SQL injection via `filter_["field"]` (HIGH data exfil), credentials redaction in server.py debug log (HIGH ops). Fixed `50ef4e9ac`: `--api-only` shutdown loop honors stop event (MEDIUM). APIv1 `sql` JSON-RPC method: NOT A FINDING — intentional feature added 2014-05-06 by PhantomPhreak (Adam Krellenstein, original creator) in commit `a29759ee8`. Live + unauthenticated on `api.counterparty.io:4000` for 12 years. Not in v2 apiary.apib. Counterparty data is public so this isn't data exfil; only DoS via expensive queries (mitigated by Cloud Armor on official node). Team decision whether to document on v2 or retire v1; nothing for this branch. |
...
| 10 | SQL invariants on snapshots | **DONE — clean** | Connected via gcloud + kubectl read-only to GKE prod mainnet pod. 0 negative balances; XCP balance = credits − debits (exact INT) for all addresses; 183 all-asset "drifts" in REAL math confirmed as double-precision ulp artifacts via INTEGER spot-check on EDRACHMA. **Mainnet ledger DB has 0 real accounting drifts across ~10 years and ~20M messages.** Bug exposure scope: dispenser oracle halt 0 live, fairminter multi-dot 0 live, subasset cap 0 historical reach (max longname 249 chars). Sub-min BTC orders post-block 286700: 8,881 would have chain-split if `5e1ba61a0` had shipped (caught by fix-review). |
...
- 2026-04-22 21:30 — Fix-review caught 1 CRITICAL + 2 HIGH + 3 MEDIUM in OUR fixes. Reverted CRITICAL (`5e1ba61a0` order.validate mirror created status double-append → consensus split). Fixed HIGH+MED: canonical_subasset_compact UnpackError swallowed by inner except (`f79b217dc`); fix_sort_bet_matches missing block_index (`f79b217dc`); BATCH_CLIENT mutex held across RPC perf regression (`183c71af6`); migration 0015 extended to also re-derive description_locked (`183c71af6`). Documented expand_subasset_longname cap reasoning (`21e5f4dce`). Big lesson: own-fix review is critical — agents catch what self-review misses.
- 2026-04-22 22:00 — 12 regression tests added (`9261884e9`) for Tier-1/Tier-2 findings + protocol gates + migrations. Fee correctness audit pass: no CRITICAL/HIGH; M-1 fairminter int hoist + L-1 issuance fee gate block_index forwarding (`3a01da244`).
- 2026-04-22 22:30 — **Empirical mainnet validation via gcloud kubectl (read-only)**. Connected to GKE public-mainnet pod. Strongest invariant — every (address, asset) balance equals exactly SUM(credits)−SUM(debits) — holds across all of mainnet history (10 years, 20M messages). Bug exposure scope: dispenser oracle 0/727, fairminter multi-dot 0/1636, subasset cap 0 historical reach. Sweep `transactions_status` confirmed at 1517/1517 (100%) NULL. Sub-min BTC orders post-block 286700: 8,881 — direct count of how many txs would have chain-split if `5e1ba61a0` had shipped. **Net: ledger accounting is empirically clean; the bugs we fixed are real but had no historical exploitation; the consensus break we caught in fix-review was 8881 rows wide.**
- 2026-04-22 23:30 — **CRITICAL btcpay vulnerability discovered + fixed.** Wave-3 audit agent's claim about btcpay missing destination check was suspected of being a 12-year false positive; user insisted on regtest/unit-test verification before shipping a fix. Test PROVED the bug: Carol crafts a btcpay paying 1 BTC to her own address with the legitimate order_match_id in OP_RETURN; parse() credits Carol with the escrowed asset; Alice loses her escrow with no BTC received. Fix `2057e099c` adds destination check gated behind new `check_btcpay_destination` (placeholder 999999999 default off). Empirical mainnet check: 0/2158 historical exploitation in 11 years — the bug has been live but unweaponized. Verifying ALSO caught 3 prior bugs in my own migration 0014/0015 code (broken import, illegal nested transaction, locked DETACH) → migrations now match the 0006 ATTACH-without-DETACH pattern (`78b5e6915`, `79d964da1`).
- 2026-04-23 00:30 — **UTXO attach OP_RETURN-vout-0 truthiness bug** verified + fixed (`49a754098`). 0 historical mainnet exposure. Gated behind `fix_attach_op_return_check` (placeholder 999999999 default off). 2 regression tests passing.
- 2026-04-23 01:00 — **UTXO "unknown" utxo_address F2 finding INVESTIGATED + CLOSED as not-a-bug.** Initial agent claim was that `safe_get_utxo_address` returning "unknown" caused detach to credit assets to a phantom address. Empirical mainnet investigation: 5 cases exist where `utxo_address='unknown'`, but ALL 5 have `address=NULL` and `utxo=<real_utxo>` — meaning the assets are at real UTXOs, just with non-derivable scriptPubKeys (bare multisig, P2PK, non-standard scripts). Tracing the actual code paths showed the 4 "utxo move" cases use `move_balances` which credits to the explicitly-specified destination UTXO (not utxo_address); the 1 "detach from utxo" case used legacy message type 100 (utxo.py) which defaults `recipient` to "first non-OP_RETURN output" of the same tx (a real UTXO), not to `balance["utxo_address"]`. The theoretical detach.py path that would produce a phantom-address credit has 0 mainnet occurrences. Conclusion: `"unknown"` is purely metadata noise on the `utxo_address` column for non-standard UTXOs — assets are spendable by whoever controls the underlying UTXO. **No fund loss; no fix warranted.** A defensive change to detach.py for a non-occurring case would be the same "fix what's not broken" pattern that produced the consensus-breaking `5e1ba61a0`.
- (next) Coordination: pick activation blocks for `fix_sort_bet_matches`, `canonical_subasset_compact`, `check_btcpay_destination`, `fix_attach_op_return_check`. The btcpay one is the most urgent — every block of delay is another window where the bug could be weaponized now that it's been documented in the public branch. Recommend rapid release with activation in ~10-100 blocks of release.
